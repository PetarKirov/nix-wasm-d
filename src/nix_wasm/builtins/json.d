...
private void jsonToNix(ref WasmAllocator allocator, const(char)[] json, ref size_t pos, out Value result) {
    skipWhitespace(json, pos);
    if (pos >= json.length) nixPanic("fromJSON: unexpected end of input");
...
private Value parseJsonString(ref WasmAllocator allocator, const(char)[] json, ref size_t pos) {
    ...
    if (buf is null) nixPanic("out of memory");
    size_t outPos = 0;

    while (pos < json.length && json[pos] != '"') {
        if (json[pos] == '\\') {
            pos++;
            if (pos >= json.length) nixPanic("fromJSON: unterminated string escape");
            switch (json[pos]) {
                ...
                default: buf[outPos++] = json[pos]; break;
            }
            pos++;
        } else {
...
private double parseDouble(const(char)[] s) {
    ...
    if (i < s.length && (s[i] == 'e' || s[i] == 'E')) {
        ...
        foreach (_; 0 .. exp) multiplier *= 10.0;
        if (expNeg)
            result /= multiplier;
        else
            result *= multiplier;
...
private Value parseJsonArray(ref WasmAllocator allocator, const(char)[] json, ref size_t pos) {
    ...
    if (items is null) nixPanic("out of memory");
    size_t count = 0;
    ...
    while (pos < json.length) {
        if (count >= MAX_ITEMS) nixPanic("fromJSON: array too large");
        Value val;
    ...
    if (pos >= json.length || json[pos] != ']') nixPanic("fromJSON: expected ']'");
    pos++;
...
private Value parseJsonObject(ref WasmAllocator allocator, const(char)[] json, ref size_t pos) {
    ...
    if (entries is null) nixPanic("out of memory");
    size_t count = 0;
    ...
    while (pos < json.length) {
        if (count >= MAX_ATTRS) nixPanic("fromJSON: object too large");

        skipWhitespace(json, pos);
        if (pos >= json.length || json[pos] != '"') nixPanic("fromJSON: expected string key");
...
private void writeJsonString(ref JsonWriter w, const(char)[] s) {
    ...
    foreach (c; s) {
        switch (c) {
            case '"': w.writeRaw("\\\""); break;
            case '\\': w.writeRaw("\\\\"); break;
            case '\b': w.writeRaw("\\b"); break;
            case '\f': w.writeRaw("\\f"); break;
            case '\n': w.writeRaw("\\n"); break;
            case '\r': w.writeRaw("\\r"); break;
            case '\t': w.writeRaw("\\t"); break;
            default:
...
private void writeDouble(ref JsonWriter w, double val) {
    ...
    if (val != val) { w.writeRaw("null"); return; } // NaN
    if (val == double.infinity) { w.writeRaw("1e308"); return; }
    if (val == -double.infinity) { w.writeRaw("-1e308"); return; }
